                ...
                <tbody>
                  ...
                  <tr v-for="t in paginatedTickets" :key="t.id" class="odd:bg-white even:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-700 flex items-center space-x-3"> 
                      <Link :href="route('tenant.tickets.show', t.id)" class="hover:text-indigo-900">
                      ...
                      <Link :href="route('tenant.tickets.edit', t.id)" class="text-black-500 mr-3">
...
const tabClass = (tab) => `pb-3 border-b-2 ${'all'===tab ? 'border-indigo-700 text-indigo-700 font-semibold' : 'border-transparent text-gray-500 hover:text-gray-700'}`;
...
const submitStatus = async () => {
  ...
  try {
    const response = await axios.post(route('tenant.tickets.status.update', { 
